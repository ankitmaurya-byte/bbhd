...
const TreeCopy: React.FC<TransferPlanTreeProps> = ({ parentRef }) => {
  // const [rootNode] = useState(hierarchy(jsondata));
  // const [pathLinks] = useState(rootNode.links());
  // const [descendantsNodes] = useState(rootNode.descendants());
  // const [textLinks] = useState(rootNode.descendants());
  ...
  useEffect(() => {
    ...
    // const descendantsDepth1 = (
    //   descendantsNodes.filter((d) => d.depth === 1)
    // );
    // const descendantsDepth2 = descendantsNodes.filter((d) => d.depth === 2);

    // const maxValue1= (
    //   max(descendantsDepth1, (d) => d.data.value || 1) || 0
    // );
    ...
    const qualityScale = new Map()<number>();
    ...
    svg
      ...
      .transition()
      .duration(1000) // Optional: Set duration for smooth transition
    ...
    //       ? rectWidth
    //       : d.depth === 1
    //       ? qualityScale1(d.data.value)
    //       : qualityScale2(d.data.value);
    //   })
    //   .attr("height", 12)
    //   .attr("fill", "#4c9875")
    //   .style("cursor", "pointer");
      ...
      const updatedChildren = json.children.map((child, index) => {
        ...
        if (child.name === d.data.name) {
          if (child.children.length === 0) {
            ...
            console.log(jsonCopy.children[index]);
          } else {
            child.children = [];
          }
        }
